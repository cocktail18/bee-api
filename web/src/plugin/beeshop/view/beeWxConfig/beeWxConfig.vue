  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeWxConfigFunc(scope.row)">变更</el-button>
...
import {
  createBeeWxConfig,
  deleteBeeWxConfig,
  deleteBeeWxConfigByIds,
  updateBeeWxConfig,
  findBeeWxConfig,
  getBeeWxConfigList
} from '@/plugin/beeshop/api/beeWxConfig'
...
const getTableData = async() => {
  const table = await getBeeWxConfigList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeWxConfigByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeWxConfigFunc = async(row) => {
    const res = await findBeeWxConfig({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeWxConfigFunc = async (row) => {
    const res = await deleteBeeWxConfig({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeWxConfig(formData.value)
                  break
                case 'update':
                  res = await updateBeeWxConfig(formData.value)
                  break
                default:
                  res = await createBeeWxConfig(formData.value)
                  break
