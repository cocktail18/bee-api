  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="评价" prop="reputation">
            
             <el-input v-model.number="searchInfo.reputation" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="评价" prop="reputation" width="120" />
        <el-table-column align="left" label="备注" prop="remark" width="120" />
        <el-table-column align="left" label="图片列表" prop="picsStr" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeOrderReputationFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            ...
            <el-form-item label="评价:"  prop="reputation" >
              <el-input v-model.number="formData.reputation" :clearable="true" placeholder="请输入评价" />
            </el-form-item>
            <el-form-item label="备注:"  prop="remark" >
              <el-input v-model="formData.remark" :clearable="true"  placeholder="请输入备注" />
            </el-form-item>
            <el-form-item label="图片列表:"  prop="picsStr" >
              <el-input v-model="formData.picsStr" :clearable="true"  placeholder="请输入图片列表" />
            </el-form-item>
          </el-form>
    </el-drawer>
  </div>
...
<script setup>
import {
  createBeeOrderReputation,
  deleteBeeOrderReputation,
  deleteBeeOrderReputationByIds,
  updateBeeOrderReputation,
  findBeeOrderReputation,
  getBeeOrderReputationList
} from '@/plugin/beeshop/api/beeOrderReputation'
...
const getTableData = async() => {
  const table = await getBeeOrderReputationList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBeeOrderReputationFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeOrderReputationByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeOrderReputationFunc = async(row) => {
    const res = await findBeeOrderReputation({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeOrderReputationFunc = async (row) => {
    const res = await deleteBeeOrderReputation({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeOrderReputation(formData.value)
                  break
                case 'update':
                  res = await updateBeeOrderReputation(formData.value)
                  break
                default:
                  res = await createBeeOrderReputation(formData.value)
                  break
