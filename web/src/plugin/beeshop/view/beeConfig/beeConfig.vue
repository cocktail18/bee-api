  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeConfigFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            ...
            <el-form-item label="value字段:"  prop="value" >
              <el-input v-model="formData.value" :clearable="true"  placeholder="请输入value字段" />
            </el-form-item>
          </el-form>
    </el-drawer>
  </div>
...
import {
  createBeeConfig,
  deleteBeeConfig,
  deleteBeeConfigByIds,
  updateBeeConfig,
  findBeeConfig,
  getBeeConfigList
} from '@/plugin/beeshop/api/beeConfig'
...
const getTableData = async() => {
  const table = await getBeeConfigList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeConfigByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeConfigFunc = async(row) => {
    const res = await findBeeConfig({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeConfigFunc = async (row) => {
    const res = await deleteBeeConfig({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeConfig(formData.value)
                  break
                case 'update':
                  res = await updateBeeConfig(formData.value)
                  break
                default:
                  res = await createBeeConfig(formData.value)
                  break
