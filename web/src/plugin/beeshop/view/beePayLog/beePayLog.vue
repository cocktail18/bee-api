  ...
  <div>
      ...
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
         ...
         <el-input v-model="searchInfo.orderNo" placeholder="搜索条件" />
           ...
           <el-form-item label="支付状态" prop="status">
            <el-select v-model="searchInfo.status" clearable placeholder="请选择" @clear="()=>{searchInfo.status=undefined}">
              <el-option v-for="(item,key) in beePayLogStatus" :key="key" :label="item.label" :value="parseInt(item.value)" />
            </el-select>
            </el-form-item>
        ...
        <template v-if="showAllQuery">
          ...
          <el-form-item label="总金额" prop="money">
            ...
            <el-input v-model.number="searchInfo.startMoney" placeholder="最小值" />
            —
            <el-input v-model.number="searchInfo.endMoney" placeholder="最大值" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeePayLogFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            ...
            <el-form-item label="支付状态:"  prop="status" >
              <el-select v-model="formData.status" placeholder="请选择支付状态" style="width:100%" :clearable="true" >
                <el-option v-for="(item,key) in beePayLogStatus" :key="key" :label="item.label" :value="parseInt(item.value)" />
              </el-select>
            </el-form-item>
            <el-form-item label="用户id:"  prop="uid" >
...
const getTableData = async() => {
  const table = await getBeePayLogList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
