    ...
    <div class="gva-form-box">
      <el-form :model="formData" ref="elFormRef" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="order_no:" prop="orderNo">
          <el-input v-model="formData.orderNo" :clearable="true"  placeholder="请输入order_no" />
       </el-form-item>
        <el-form-item label="pay_gate:" prop="payGate">
          <el-input v-model="formData.payGate" :clearable="true"  placeholder="请输入pay_gate" />
       </el-form-item>
        ...
        <el-form-item label="支付状态:" prop="status">
           <el-select v-model="formData.status" placeholder="请选择支付状态" style="width:100%" :clearable="true" >
              <el-option v-for="(item,key) in pay_log_statusOptions" :key="key" :label="item.label" :value="item.value" />
           </el-select>
       </el-form-item>
...
const type = ref('')
const pay_log_statusOptions = ref([])
const formData = ref({
            id: undefined,
            userId: undefined,
            isDeleted: false,
...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findBeePayLog({ ID: route.query.id })
      if (res.code === 0) {
    ...
    pay_log_statusOptions.value = await getDictFunc('pay_log_status')
}
