  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="所属属性" prop="propertyId">
          <el-select
              v-model="searchInfo.propertyId"
              :clearable="true"
          >
            <el-option
                v-for="(item,key) in propCategoryList"
                :key="item.id"
                :value="item.id"
                :label="item.name"
            />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeShopGoodsPropFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createBeeShopGoodsProp,
  deleteBeeShopGoodsProp,
  deleteBeeShopGoodsPropByIds,
  updateBeeShopGoodsProp,
  findBeeShopGoodsProp,
  getBeeShopGoodsPropList
} from '@/plugin/beeshop/api/beeShopGoodsProp'
...
const init = async() => {
  const table = await getBeeShopGoodsPropList({ page: 1, pageSize: 10000, propertyId: 0 })
  if (table.code === 0) {
    propCategoryList.value = table.data.list
  }
}
...
const getTableData = async() => {
  init()
  const table = await getBeeShopGoodsPropList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBeeShopGoodsPropFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeShopGoodsPropByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeShopGoodsPropFunc = async(row) => {
    const res = await findBeeShopGoodsProp({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeShopGoodsPropFunc = async (row) => {
    const res = await deleteBeeShopGoodsProp({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeShopGoodsProp(formData.value)
                  break
                case 'update':
                  res = await updateBeeShopGoodsProp(formData.value)
                  break
                default:
                  res = await createBeeShopGoodsProp(formData.value)
                  break
