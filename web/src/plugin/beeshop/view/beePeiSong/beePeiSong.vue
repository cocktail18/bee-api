<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="距离，单位km" prop="distance" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeePeiSongFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            <el-form-item label="距离，单位km:"  prop="distance" >
              <el-input-number v-model="formData.distance"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
...
const getTableData = async() => {
  const table = await getBeePeiSongList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeePeiSongByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeePeiSongFunc = async(row) => {
    const res = await findBeePeiSong({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeePeiSongFunc = async (row) => {
    const res = await deleteBeePeiSong({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
