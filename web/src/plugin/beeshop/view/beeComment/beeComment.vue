<template>
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="关联id" prop="refId">
             ...
             <el-input v-model.number="searchInfo.refId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="关联id" prop="refId" width="120" />
        <el-table-column align="left" label="父评论id" prop="pid" width="120" />
        <el-table-column align="left" label="商店id" prop="shopId" width="120" />
        <el-table-column align="left" label="类型" prop="type" width="120" />
        <el-table-column align="left" label="图片" prop="pics" width="120">
          <template #default="scope">
            <template v-for="(item,index) in scope.row.pics.split(',')" :key="index">
              <CustomPic
                  v-if="item"
                  pic-type="file"
        ...
        <el-table-column align="left" label="文件" prop="file" width="120" />
        <el-table-column align="left" label="手机号" prop="mobile" width="120" />
        <el-table-column align="left" label="内容" prop="content" width="120" />
        <el-table-column align="left" label="额外json信息" prop="extJsonStr" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeCommentFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            ...
            <el-form-item label="关联id:"  prop="refId" >
              <el-input v-model.number="formData.refId" :clearable="true" placeholder="请输入关联id" />
            </el-form-item>
            <el-form-item label="父评论id:"  prop="pid" >
              <el-input v-model.number="formData.pid" :clearable="true" placeholder="请输入父评论id" />
            </el-form-item>
            <el-form-item label="商店id:"  prop="shopId" >
...
const getTableData = async() => {
  const table = await getBeeCommentList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeCommentByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeCommentFunc = async(row) => {
    const res = await findBeeComment({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeCommentFunc = async (row) => {
    const res = await deleteBeeComment({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
