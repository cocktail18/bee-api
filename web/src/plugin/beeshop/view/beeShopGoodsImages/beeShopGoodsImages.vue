  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeShopGoodsImagesFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createBeeShopGoodsImages,
  deleteBeeShopGoodsImages,
  deleteBeeShopGoodsImagesByIds,
  updateBeeShopGoodsImages,
  findBeeShopGoodsImages,
  getBeeShopGoodsImagesList
} from '@/plugin/beeshop/api/beeShopGoodsImages'
...
const getTableData = async() => {
  const table = await getBeeShopGoodsImagesList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBeeShopGoodsImagesFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeShopGoodsImagesByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeShopGoodsImagesFunc = async(row) => {
    const res = await findBeeShopGoodsImages({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeShopGoodsImagesFunc = async (row) => {
    const res = await deleteBeeShopGoodsImages({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeShopGoodsImages(formData.value)
                  break
                case 'update':
                  res = await updateBeeShopGoodsImages(formData.value)
                  break
                default:
                  res = await createBeeShopGoodsImages(formData.value)
                  break
