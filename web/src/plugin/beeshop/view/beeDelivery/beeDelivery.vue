  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="app_key" prop="appKey" width="120" />
        <el-table-column align="left" label="app_secret" prop="appSecret" width="120" />
        <el-table-column align="left" label="source_id" prop="sourceId" width="120" />
        <el-table-column align="left" label="debug模式" prop="debug" width="120">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeDeliveryFunc(scope.row)">变更</el-button>
...
<!--        </el-form-item>-->
        ...
        <el-form-item label="app_key:"  prop="appKey" >
          <el-input v-model="formData.appKey" :clearable="true"  placeholder="请输入app_key" />
        </el-form-item>
        ...
        <el-form-item label="source_id:"  prop="sourceId" >
          <el-input v-model="formData.sourceId" :clearable="true"  placeholder="请输入source_id" />
        </el-form-item>
        <el-form-item label="debug模式:"  prop="debug" >
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    ...
    if (searchInfo.value.debug === ""){
      searchInfo.value.debug=null
    }
    getTableData()
  })
}
...
const getTableData = async() => {
  const table = await getBeeDeliveryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    const res = await deleteBeeDeliveryByIds({ ids })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
...
// 更新行
const updateBeeDeliveryFunc = async(row) => {
  const res = await findBeeDelivery({ id: row.id })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteBeeDeliveryFunc = async (row) => {
  const res = await deleteBeeDelivery({ id: row.id })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async () => {
  elFormRef.value?.validate( async (valid) => {
    ...
    switch (type.value) {
      case 'create':
        res = await createBeeDelivery(formData.value)
        break
      case 'update':
        res = await updateBeeDelivery(formData.value)
        break
      default:
        res = await createBeeDelivery(formData.value)
        break
