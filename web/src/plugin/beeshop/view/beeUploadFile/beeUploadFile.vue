<template>
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="过期时间" prop="expireAt">
            ...
            <el-input v-model.number="searchInfo.startExpireAt" placeholder="最小值" />
            —
            <el-input v-model.number="searchInfo.endExpireAt" placeholder="最大值" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="domain" prop="domain" width="120" />
        <el-table-column align="left" label="文件名" prop="filename" width="120" />
        <el-table-column align="left" label="保存路径" prop="dst" width="120" />
        <el-table-column align="left" label="过期时间" prop="expireAt" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeUploadFileFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            <el-form-item label="domain:"  prop="domain" >
              <el-input v-model="formData.domain" :clearable="true"  placeholder="请输入domain" />
            </el-form-item>
            <el-form-item label="文件名:"  prop="filename" >
              <el-input v-model="formData.filename" :clearable="true"  placeholder="请输入文件名" />
            </el-form-item>
            <el-form-item label="保存路径:"  prop="dst" >
              <el-input v-model="formData.dst" :clearable="true"  placeholder="请输入保存路径" />
            </el-form-item>
            <el-form-item label="过期时间:"  prop="expireAt" >
              <el-input v-model.number="formData.expireAt" :clearable="true" placeholder="请输入过期时间" />
            </el-form-item>
          </el-form>
    </el-drawer>
  </div>
...
import {
  createBeeUploadFile,
  deleteBeeUploadFile,
  deleteBeeUploadFileByIds,
  updateBeeUploadFile,
  findBeeUploadFile,
  getBeeUploadFileList
} from '@/plugin/beeshop/api/beeUploadFile'
...
const getTableData = async() => {
  const table = await getBeeUploadFileList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeUploadFileByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeUploadFileFunc = async(row) => {
    const res = await findBeeUploadFile({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeUploadFileFunc = async (row) => {
    const res = await deleteBeeUploadFile({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeUploadFile(formData.value)
                  break
                case 'update':
                  res = await updateBeeUploadFile(formData.value)
                  break
                default:
                  res = await createBeeUploadFile(formData.value)
                  break
