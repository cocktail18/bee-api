<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="模板名" prop="name" width="120" />
        <el-table-column align="left" label="计价方式" prop="feeType" width="120" >
          <template #default="scope">{{ formatEnum(scope.row.feeType, beeGoodsFeeType) }}</template>
        </el-table-column>
        <el-table-column align="left" label="是否包邮" prop="isFree" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.isFree) }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeLogisticsFunc(scope.row)">变更</el-button>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="计价方式:"  prop="feeType" >
              <el-select v-model="formData.feeType" clearable placeholder="请选择" :clearable="false">
                <el-option v-for="(item,key) in beeGoodsFeeType" :key="key" :label="item.label"
                           :value="parseInt(item.value)"/>
              </el-select>
            </el-form-item>
            <el-form-item label="是否包邮:"  prop="isFree" >
              <el-switch v-model="formData.isFree" active-color="#13ce66" inactive-color="#ff4949" active-text="是" inactive-text="否" clearable ></el-switch>
            </el-form-item>
...
const beeGoodsFeeType = ref([])
const init = async () => {
  beeGoodsFeeType.value = await getDictFunc('BeeGoodsFeeType')
}
init()
// 自动化生成的字典（可能为空）以及字段
const formData = ref({
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    ...
    page.value = 1
    pageSize.value = 10
    if (searchInfo.value.isFree === ""){
        searchInfo.value.isFree=null
    }
...
const getTableData = async() => {
  const table = await getBeeLogisticsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeLogisticsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeLogisticsFunc = async(row) => {
  router.push('beeLogisticsForm?id='+row.id);
    // const res = await findBeeLogistics({ id: row.id })
    // type.value = 'update'
    // if (res.code === 0) {
...
// 删除行
const deleteBeeLogisticsFunc = async (row) => {
    const res = await deleteBeeLogistics({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
