  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="source字段" prop="source">
             ...
             <el-input v-model.number="searchInfo.source" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="openId字段" prop="openId">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="来源" prop="source" width="120">
          <template #default="scope">{{ formatEnum(scope.row.source, userSourceTypeMap) }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeUserMapperFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            ...
            <el-form-item label="来源:"  prop="source" >
              <el-select v-model="formData.source" placeholder="请选择来源" :clearable="false" style="width:100%">
                <el-option
                  v-for="(item,key) in userSourceTypeMap"
                  :key="key"
                  :label="item.label"
            ...
            <el-form-item label="openId:"  prop="openId" >
              <el-input v-model="formData.openId" :clearable="true"  placeholder="请输入openId字段" />
            </el-form-item>
            <el-form-item label="unionId:"  prop="unionId" >
              <el-input v-model="formData.unionId" :clearable="true"  placeholder="请输入unionId字段" />
            </el-form-item>
          </el-form>
    </el-drawer>
  </div>
...
import {
  createBeeUserMapper,
  deleteBeeUserMapper,
  deleteBeeUserMapperByIds,
  updateBeeUserMapper,
  findBeeUserMapper,
  getBeeUserMapperList
} from '@/plugin/beeshop/api/beeUserMapper'
...
const userSourceTypeMap = ref([])
const init = async () => {
  userSourceTypeMap.value = await getDictFunc('BeeUserSource')
  console.log(userSourceTypeMap)
}
init()
// 自动化生成的字典（可能为空）以及字段
const formData = ref({
...
const getTableData = async() => {
  const table = await getBeeUserMapperList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeUserMapperByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeUserMapperFunc = async(row) => {
    const res = await findBeeUserMapper({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeUserMapperFunc = async (row) => {
    const res = await deleteBeeUserMapper({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeUserMapper(formData.value)
                  break
                case 'update':
                  res = await updateBeeUserMapper(formData.value)
                  break
                default:
                  res = await createBeeUserMapper(formData.value)
                  break
