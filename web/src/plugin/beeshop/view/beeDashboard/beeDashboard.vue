<template>
  <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 py-2 gap-4 md:gap-2 gva-container2">
    <gva-card custom-class="col-span-1 lg:col-span-2 h-32" @click="gotoPage('../beeUser/beeUser.vue')">
    ...
    <gva-card title="待办事项" custom-class="col-span-1 md:col-span-6 lg:col-span-6 row-span-2">
      <div>
        <el-table :data="orderTodo.list" stripe style="width: 100%">
...
const init = async () => {
  ...
  for (let i = 7; i >= 0; i--) {
    ...
    const userTable = await getBeeUserList({page: 1, pageSize: 1, startDateAdd: start, endDateAdd: end})
    if (userTable.code === 0) {
      userData.value.push(userTable.data.total)
    }
    const payLogUidTable = await getBeePayLogList({page: 1, pageSize: 1, startDateAdd: start, endDateAdd: end, distinct: 'uid'})
    if (payLogUidTable.code === 0) {
      payNumData.value.push(payLogUidTable.data.total)
    }
    const orderTable = await getBeeOrderList({page: 1, pageSize: 1, startDateAdd: start, endDateAdd: end})
    if (orderTable.code === 0) {
      orderData.value.push(orderTable.data.total)
    }
    const goodsTable = await getBeeShopGoodsList({page: 1, pageSize: 1, startDateAdd: start, endDateAdd: end})
    if (goodsTable.code === 0) {
      goodsData.value.push(goodsTable.data.total)
    }
    const payDataTable = await getBeePayTotal({page: 1, pageSize: 1, startDateAdd: start, endDateAdd: end, sum: 'money'})
