  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeUserAddressFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteBeeUserAddress,
  deleteBeeUserAddressByIds,
  updateBeeUserAddress,
  findBeeUserAddress,
  getBeeUserAddressList
...
const getTableData = async() => {
  const table = await getBeeUserAddressList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeUserAddressByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeUserAddressFunc = async(row) => {
    const res = await findBeeUserAddress({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeUserAddressFunc = async (row) => {
    const res = await deleteBeeUserAddress({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
