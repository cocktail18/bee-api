<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="剩余可用余额" prop="balance" width="120" />
        <el-table-column align="left" label="收入/支出" prop="behavior" width="120" >
          <template #default="scope">{{ formatEnum(scope.row.behavior, beeCashLogBehavior) }}</template>
        </el-table-column>
        <el-table-column align="left" label="剩余冻结金额" prop="freeze" width="120" />
        <el-table-column align="left" label="remark" prop="remark" width="120" />
        <el-table-column align="left" label="类型" prop="type" width="120" >
          <template #default="scope">{{ formatEnum(scope.row.type, beeCashLogType) }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeCashLogFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            ...
            <el-form-item label="收入/支出:"  prop="behavior" >
              <el-select v-model="formData.behavior" clearable placeholder="请选择" :clearable="false">
                <el-option v-for="(item,key) in beeCashLogBehavior" :key="key" :label="item.label" :value="parseInt(item.value)" />
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="消费类型:"  prop="type" >
              <el-select v-model="formData.type" clearable placeholder="请选择" :clearable="false">
                <el-option v-for="(item,key) in beeCashLogType" :key="key" :label="item.label" :value="parseInt(item.value)" />
              </el-select>
            </el-form-item>
...
const beeCashLogBehavior = ref([])
const beeCashLogType = ref([])
const init = async () => {
  beeCashLogBehavior.value = await getDictFunc('BeeCashLogBehavior')
  beeCashLogType.value = await getDictFunc('BeeCashLogType')
}
init()
...
const getTableData = async() => {
  const table = await getBeeCashLogList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeCashLogByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeCashLogFunc = async(row) => {
    const res = await findBeeCashLog({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeCashLogFunc = async (row) => {
    const res = await deleteBeeCashLog({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
