<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="商品描述" prop="content" width="120" >
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeShopGoodsContentFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            <el-form-item label="商品id:"  prop="goodsId" >
              <el-input v-model.number="formData.goodsId" :clearable="true" placeholder="商品id" />
            </el-form-item>
...
<script setup>
import {
  createBeeShopGoodsContent,
  deleteBeeShopGoodsContent,
  deleteBeeShopGoodsContentByIds,
  updateBeeShopGoodsContent,
  findBeeShopGoodsContent,
  getBeeShopGoodsContentList
} from '@/plugin/beeshop/api/beeShopGoodsContent'
...
const getTableData = async() => {
  const table = await getBeeShopGoodsContentList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBeeShopGoodsContentFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeShopGoodsContentByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeShopGoodsContentFunc = async(row) => {
    const res = await findBeeShopGoodsContent({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeShopGoodsContentFunc = async (row) => {
    const res = await deleteBeeShopGoodsContent({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeShopGoodsContent(formData.value)
                  break
                case 'update':
                  res = await updateBeeShopGoodsContent(formData.value)
                  break
                default:
                  res = await createBeeShopGoodsContent(formData.value)
                  break
