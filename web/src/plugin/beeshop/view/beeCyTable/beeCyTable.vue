    ...
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="tableNum字段" prop="tableNum">
         <el-input v-model="searchInfo.tableNum" placeholder="搜索条件" />
...
const getTableData = async() => {
  const table = await getBeeCyTableList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeCyTableByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeCyTableFunc = async(row) => {
    const res = await findBeeCyTable({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
const deleteBeeCyTableFunc = async (row) => {
    const res = await deleteBeeCyTable({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeCyTable(formData.value)
                  break
                case 'update':
                  res = await updateBeeCyTable(formData.value)
                  break
                default:
                  res = await createBeeCyTable(formData.value)
                  break
