  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="类型" prop="type" width="120" >
          <template #default="scope">{{ formatEnum(scope.row.type, beeOrderLogType) }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeOrderLogFunc(scope.row)">变更</el-button>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="类型:"  prop="type" >
              <el-select v-model="formData.type" clearable placeholder="请选择" :clearable="false">
                <el-option v-for="(item,key) in beeOrderLogType" :key="key" :label="item.label"
...
const getTableData = async() => {
  const table = await getBeeOrderLogList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeOrderLogByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeOrderLogFunc = async(row) => {
    const res = await findBeeOrderLog({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeOrderLogFunc = async (row) => {
    const res = await deleteBeeOrderLog({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
