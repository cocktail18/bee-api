<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="业务id" prop="businessId" width="120" />
        <el-table-column align="left" label="链接类型" prop="linkType" width="120" />
        <el-table-column sortable align="left" label="排序" prop="paixu" width="120" />
        <el-table-column align="left" label="图片" prop="picUrl" width="120" >
          <template #default="scope">
            <CustomPic
                pic-type="file"
                style="margin-top:8px"
                :pic-src="scope.row.picUrl"
            />
          </template>
        </el-table-column>
        <el-table-column sortable align="left" label="店铺id" prop="shopId" width="120" />
        ...
        <el-table-column sortable align="left" label="类型" prop="type" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeBannerFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            <el-form-item label="业务id:"  prop="businessId" >
              <el-input v-model.number="formData.businessId" :clearable="true" placeholder="请输入业务id" />
            </el-form-item>
            <el-form-item label="链接类型:"  prop="linkType" >
              <el-input v-model.number="formData.linkType" :clearable="true" placeholder="请输入链接类型" />
            </el-form-item>
            <el-form-item label="排序:"  prop="paixu" >
              <el-input v-model.number="formData.paixu" :clearable="true" placeholder="请输入排序" />
            </el-form-item>
            <el-form-item label="图片:"  prop="picUrl" >
              <el-input v-model="formData.picUrl" :clearable="true"  placeholder="请输入图片" />
              <SelectImage
...
import {
  createBeeBanner,
  deleteBeeBanner,
  deleteBeeBannerByIds,
  updateBeeBanner,
  findBeeBanner,
  getBeeBannerList
} from '@/plugin/beeshop/api/beeBanner'
...
const getTableData = async() => {
  const table = await getBeeBannerList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeBannerByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeBannerFunc = async(row) => {
    const res = await findBeeBanner({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeBannerFunc = async (row) => {
    const res = await deleteBeeBanner({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeBanner(formData.value)
                  break
                case 'update':
                  res = await updateBeeBanner(formData.value)
                  break
                default:
                  res = await createBeeBanner(formData.value)
                  break
