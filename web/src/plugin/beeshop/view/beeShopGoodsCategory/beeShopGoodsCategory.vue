<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="是否启用" prop="isUse" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.isUse) }}</template>
        </el-table-column>
...
<!--        <el-table-column align="left" label="店铺id" prop="shopId" width="120" />-->
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeShopGoodsCategoryFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteBeeShopGoodsCategory,
  deleteBeeShopGoodsCategoryByIds,
  updateBeeShopGoodsCategory,
  findBeeShopGoodsCategory,
  getBeeShopGoodsCategoryList
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    ...
    if (searchInfo.value.isUse === ""){
        searchInfo.value.isUse=null
    }
    getTableData()
  })
}
...
const getTableData = async() => {
  const table = await getBeeShopGoodsCategoryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBeeShopGoodsCategoryFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeShopGoodsCategoryByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeShopGoodsCategoryFunc = async(row) => {
    const res = await findBeeShopGoodsCategory({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeShopGoodsCategoryFunc = async (row) => {
    const res = await deleteBeeShopGoodsCategory({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
