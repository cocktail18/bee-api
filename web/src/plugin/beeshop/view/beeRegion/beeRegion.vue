<template>
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="firstLetter字段" prop="firstLetter">
         <el-input v-model="searchInfo.firstLetter" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="jianpin字段" prop="jianpin">
         <el-input v-model="searchInfo.jianpin" placeholder="搜索条件" />
        ...
        <el-form-item label="pinyin字段" prop="pinyin">
         <el-input v-model="searchInfo.pinyin" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="firstLetter字段" prop="firstLetter" width="120" />
        <el-table-column align="left" label="jianpin字段" prop="jianpin" width="120" />
        <el-table-column align="left" label="level字段" prop="level" width="120" />
        <el-table-column align="left" label="名称" prop="name" width="120" />
        <el-table-column align="left" label="nameEn字段" prop="nameEn" width="120" />
        <el-table-column align="left" label="pinyin字段" prop="pinyin" width="120" />
        <el-table-column sortable align="left" label="pid字段" prop="pid" width="120" />
        <el-table-column align="left" label="regionId字段" prop="regionId" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeRegionFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            ...
            <el-form-item label="level字段:"  prop="level" >
              <el-input v-model.number="formData.level" :clearable="true" placeholder="请输入level字段" />
            </el-form-item>
            ...
            <el-form-item label="regionId字段:"  prop="regionId" >
              <el-input v-model.number="formData.regionId" :clearable="true" placeholder="请输入regionId字段" />
            </el-form-item>
          </el-form>
    </el-drawer>
  </div>
...
const getTableData = async() => {
  const table = await getBeeRegionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeRegionByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeRegionFunc = async(row) => {
    const res = await findBeeRegion({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeRegionFunc = async (row) => {
    const res = await deleteBeeRegion({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
