...
const task_result_query_url = "/waimai/ng/valueadded/async/taskResultQuery"
...
type TaskResultQueryRequest struct {
    /**
    *  枚举 2 根据系统标签获取用户列表 3 根据自定以标签获取用户列表 4 建券并发券 5 手机号发劵 
    */
...
func (req *TaskResultQueryRequest) DoInvoke(client mtclient.MeituanClient, appAuthToken string) (*TaskResultQueryResponse, error) {
    resp, err := client.InvokeApi(task_result_query_url, 2, appAuthToken, req)
